...
class moduleCL(nn.Module):
    def __init__(self,
                 input_size, hidden_size, head, device, args):
    ...
    def forward(self, hidden, reward, keyused, obs, time, seed, doimage):
        ...
        for iii in range(self.head):
            out1[iii] = self.layers[iii](self.main[iii](hidden))
            ...
            ortho2[iii] = F.softmax(cossim2[iii].squeeze() * 100., dim=0)

        cossimtotal = torch.stack(cossim2,  dim=2)
        orthototal = torch.stack(ortho2, dim=2)
        cossimtotalmaxxx, indmaxes = ( torch.max(cossimtotal, dim=0))  # 16,3
