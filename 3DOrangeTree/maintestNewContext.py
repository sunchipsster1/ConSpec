...
def main():
    ...
    envU = UnityEnvironment(r"[directory...]/ProjTreeSimplePink2extrahardLinux4590_338084842TestmissRoommgp/GridWorld.x86_64",seed=2,worker_id=1,no_graphics=False) #ProjTreeSimplePink2extrahardLinux4590_338084842Testmissb
    ...
    moduleuse.to(device)
    actor_criticCL = torch.load(os.path.join(save_path, "modelCL"+ str(args.algo )  + 'HEAD' + str(args.head)+ 'factor' + str(args.factorR)  + str(args.seed) + "seed1000.pt"))#modelCLxxxxOneshot35seed776 modelCLxxxxOneshot31seed385  modelCLxxxxOneshot32seed1499 modelCLxxxxOneshot33seed860 #modelCLxxxxOneshot34seed941
    actor_critic = torch.load(os.path.join(save_path, "model" + str(args.algo )  + 'HEAD' + str(args.head)+ 'factor' + str(args.factorR) + str(args.seed) + "seed1000.pt"))#modelxxxxOneshot35seed776 modelxxxxOneshot31seed385  modelxxxxOneshot32seed1499 modelxxxxOneshot33seed860 #modelxxxxOneshot34seed941
    moduleuse = torch.load(os.path.join(save_path, "moduleuse" + 'HEAD' + str(args.head) + str(args.algo) + 'factor' + str(args.factorR) + str(args.seed) + "seed1000.pt"))
    keysUsedt = torch.FloatTensor(keysUsed).to(device=device)

    if args.algo == 'Conspec':
        agent = algo.PPOConspectest(
    ...
    for j in range(20):
        ...
        for kkk in range(minibsize):
            ...
            for step in range(args.num_steps):
                ...
                if donetotal < 2.5:
                    ...
                    reward = np.floor(reward / 5.) / 10.
                    reward = torch.from_numpy((reward).reshape(-1, 1)).reshape(-1, 1)  #####################
                    done = np.ones((1,)) * done  #####################
                    reward = reward * maskstotal  # * 1./1000.
        ...
        try:
            ...
            with torch.no_grad():
                ...
                episode_rewards.append(rewardstotal[-22:].sum(0).mean().cpu().detach().numpy())
                ...
                print(
                    "Updates {}, num timesteps {}, FPS {} \n Last {} training episodes: mean/median reward {:.1f}/{:.1f}, min/max reward {:.1f}/{:.1f}\n"
                        .format(j, total_num_steps,
                                ...
                                np.max(episode_rewards), 0, 0,
                                0))
                LOGFILE = './ResultsNewContext' + str(args.env_name) + 'choiceCLparam' + str(
                    args.choiceCLparams) + 'lrCL' + str(args.lrCL)+ 'factor' + str(args.factorR)  + 'seed' + str(args.seed) + '.txt'
                print(LOGFILE)
