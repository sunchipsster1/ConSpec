...
def main():
    ...
    envU = UnityEnvironment(r"[directory...]/ProjTreeSimplePink2extrahardLinux4590_33_84842onevarb/GridWorld.x86_64",seed=2,worker_id=1,no_graphics=False) #ProjTreeSimplePink2extrahardLinux4590_338084842Testmissb
    ...
    moduleuse.to(device)
    actor_criticCL = torch.load(os.path.join(save_path, "modelCL" + str(args.algo) + 'HEAD' + str(
        args.head) + 'factor' + str(args.factorR) + str(
        args.seed) + "seed1000.pt"))  # modelCLxxxxOneshot35seed776 modelCLxxxxOneshot31seed385  modelCLxxxxOneshot32seed1499 modelCLxxxxOneshot33seed860 #modelCLxxxxOneshot34seed941
    actor_critic = torch.load(os.path.join(save_path, "model" + str(args.algo) + 'HEAD' + str(
        args.head) + 'factor' + str(args.factorR) + str(
        args.seed) + "seed1000.pt"))  # modelxxxxOneshot35seed776 modelxxxxOneshot31seed385  modelxxxxOneshot32seed1499 modelxxxxOneshot33seed860 #modelxxxxOneshot34seed941
    moduleuse = torch.load(os.path.join(save_path, "moduleuse" + 'HEAD' + str(args.head) + str(
        args.algo) + 'factor' + str(args.factorR) + str(args.seed) + "seed1000.pt"))
                ...
                print(
                    "Updates {}, num timesteps {}, FPS {} \n Last {} training episodes: mean/median reward {:.1f}/{:.1f}, min/max reward {:.1f}/{:.1f}\n"
                        .format(j, total_num_steps,
                                ...
                                0))
                LOGFILE = './Results_blackobj' + str(args.env_name) + 'choiceCLparam' + str(
