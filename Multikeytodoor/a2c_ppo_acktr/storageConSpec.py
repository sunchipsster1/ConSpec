...
class RolloutStorage(object):
    ...
    def correctR(self):
        totalreward = self.rewards.sum(0)  # [500, 8, 1]
        totalreward = totalreward.squeeze()
        rewardssortbad = torch.nonzero(totalreward > -50.).reshape(-1, )
        if rewardssortbad.shape[0] > 0:
            self.rewards[-2, rewardssortbad, :] = -30000.
        return self.rewards
